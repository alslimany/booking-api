...
class FlighOfferSearchController extends Controller
{
    ...
    public function create_order(Request $request)
    {
        ...
        foreach ($data['travelers'] as $traveler) {
            ...
            if ($traveler['type'] == "seated_infant") {
                $issue_pnr_command .= "-1" . $traveler['name']['lastName'] . "/" . $traveler['name']['firstName'] . 'MSTR.ISN06^';
            }
    ...
    private function oneway_flight_offers(Request $request)
    {
        ...
        $direct_flight_schedules = FlightSchedule::whereDate('departure', '=', date('Y-m-d', strtotime($request->departure_date)))
